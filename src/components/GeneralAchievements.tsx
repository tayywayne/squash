    ...
    return (
      <div className={`bg-white p-6 border-3 border-black shadow-brutal ${className}`}>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-black text-dark">ACHIEVEMENTS</h3>
        </div>
  ...
  return (
    <div className={`bg-white p-6 border-3 border-black shadow-brutal ${className}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-1 text-sm text-dark">
          <Trophy size={16} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {achievements.map((achievement, index) => (
          <div
            ...
            className="group relative bg-accent p-4 border-3 border-black hover:shadow-brutal transition-all duration-200 cursor-pointer"
          >
            {/* Achievement Badge */}
            <div className="text-center">
              <div className="text-2xl mb-2">{achievement.emoji}</div>
              <div className="text-xs font-black text-text leading-tight">
                {achievement.name}
              </div>
            </div>

            {/* Unlock Date */}
            <div className="flex items-center justify-center mt-2 text-xs text-text">
            ...
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
              <div className="bg-dark text-text text-xs border-3 border-black p-4 shadow-brutal w-80 max-w-sm">
                <div className="font-black mb-1 text-accent">
                  {achievement.name}
                </div>
