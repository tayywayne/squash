/*
  # Create quest-related database functions

  1. Functions Created
    - `get_available_quests` - Returns all available quests with user progress
    - `get_quest_details` - Returns detailed quest information with steps
    - `start_quest` - Starts a new quest for a user
    - `submit_quest_step` - Submits a quest step response
  ...
  3. Type Safety
    - All numeric columns properly cast to INTEGER to avoid bigint/integer mismatches
    - Consistent return types matching frontend expectations
*/
...
BEGIN
  ...
  SELECT 
    ...
    q.description,
    q.emoji,
    q.reward_cred::INTEGER,
    q.unlocks_tool,
    q.difficulty,
    q.theme,
    COALESCE(uq.id IS NOT NULL, false) as is_started,
    COALESCE(uq.is_completed, false) as is_completed,
    COALESCE(uq.current_step, 1)::INTEGER as current_step,
    COALESCE(step_counts.total_steps, 0)::INTEGER as total_steps,
    CASE 
      WHEN uq.is_completed THEN 100
      WHEN uq.id IS NULL THEN 0
      ELSE ROUND((COALESCE(uq.current_step, 1)::FLOAT / COALESCE(step_counts.total_steps, 1)::FLOAT) * 100)::INTEGER
