/*
  # Update leaderboard function to include supporter_emoji

  1. Changes
    - Update the `get_user_conflict_stats` function to include `supporter_emoji` in the return type
    - Add `supporter_emoji` to the SELECT statement in the function body
    - This will allow the leaderboard to display user supporter emojis alongside archetype emojis

  2. Function Updates
    - Add `supporter_emoji text` to the RETURNS TABLE definition
    - Include `p.supporter_emoji` in the SELECT statements within the function
*/

-- Drop and recreate the function with supporter_emoji support
DROP FUNCTION IF EXISTS public.get_user_conflict_stats(timestamptz);

-- Create updated function to get user conflict statistics with supporter emoji
...
BEGIN
  ...
  WITH user_stats AS (
    ...
    GROUP BY p.id, p.username, p.first_name, p.last_name, p.avatar_url, p.archetype_emoji, p.supporter_emoji
