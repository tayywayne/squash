/*
  # Fix Quest Functions Parameter Names

  1. Changes
    - Update parameter names in all quest-related functions to match frontend expectations
    - Change from user_id_param to p_user_id
    - Change from quest_id_param to p_quest_id
    - Change from user_quest_id_param to p_user_quest_id
    - Change from step_id_param to p_step_id
    - Change from user_response_param to p_user_response

  2. Security
    - Maintain existing SECURITY DEFINER settings
    - Preserve all function logic
    - Only change parameter names to match frontend expectations
*/
...
-- Recreate get_available_quests with p_user_id parameter
CREATE OR REPLACE FUNCTION get_available_quests(p_user_id uuid DEFAULT NULL)
RETURNS TABLE (
  id uuid,
  title text,
  description text,
...
AS $$
BEGIN
  -- Use the provided p_user_id or fall back to auth.uid()
...
-- Recreate get_quest_details with p_quest_id and p_user_id parameters
...
BEGIN
  SELECT json_build_object(
    'quest', json_build_object(
      ...
      'emoji', q.emoji,
      'reward_cred', q.reward_cred::INTEGER,
    ...
    'user_progress', json_build_object(
      ...
      'current_step', COALESCE(uq.current_step, 1)::INTEGER,
  ...
  LEFT JOIN (
    SELECT 
      ...
      json_agg(
        json_build_object(
          'id', qs.id,
          'step_number', qs.step_number::INTEGER,
          'title', qs.title,
...
-- Recreate start_quest with p_user_id and p_quest_id parameters
...
-- Recreate submit_quest_step with p_user_quest_id, p_step_id, and p_user_response parameters
...
BEGIN
  ...
  SELECT COUNT(*)::INTEGER INTO total_steps
  FROM quest_steps
