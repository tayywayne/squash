/*
  ...
  1. Changes
    - Update the `submit_quest_step` function to properly unlock achievements when a quest is completed
    - Add `is_quest_achievement_unlocked` boolean to the return JSON
    - Add `quest_id`, `quest_title`, and `quest_difficulty` to the return JSON
    - Ensure the correct emoji from the quest is passed to the achievement
    - Fix the award_squashcred call to use the correct user ID
...
DECLARE
  ...
  achievement_result BOOLEAN;
  is_achievement_unlocked BOOLEAN := false;
  achievement_code TEXT;
  quest_info RECORD;
BEGIN
  -- Get step details
  SELECT qs.*, q.reward_cred, q.id as quest_id, q.title as quest_title, q.difficulty, q.emoji as quest_emoji
  INTO step_record
  FROM quest_steps qs
  ...
  IF next_step > total_steps THEN
    ...
    -- Unlock achievement
    SELECT unlock_user_achievement(
      user_quest_record.user_id,
      achievement_code,
      'Quest: ' || step_record.quest_title,
      step_record.quest_emoji,
