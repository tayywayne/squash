/*
  # Fix get_user_conflict_stats function to include archetype_emoji

  1. Changes
    - Update the `get_user_conflict_stats` function to include `archetype_emoji` in the return type
    - Add `archetype_emoji` to the SELECT statement in the function body
    - This will allow the leaderboard to display user archetype emojis

  2. Function Updates
    - Add `archetype_emoji text` to the RETURNS TABLE definition
    - Include `p.archetype_emoji` in the SELECT statements within the function
*/

-- Drop and recreate the function with archetype_emoji support
DROP FUNCTION IF EXISTS public.get_user_conflict_stats(timestamptz);

-- Create updated function to get user conflict statistics with archetype emoji
